...
def get_loans(service: Annotated[LoanService, Depends(get_loan_service)],
              ...
              overdue: Annotated[bool, Query(description="Lists only overdue loans")] = False) -> MemoryPaginatedResponse:
...
def create_loan(book_id: Annotated[UUID,
                                   UuidVersion(4),
                                   Query(description="Book identifier",
                                         examples=["292ada19-6b06-44f7-a20e-dc6687668b0e"])],
                user_id: Annotated[UUID,
                                   UuidVersion(4),
                                   Query(description="User identifier",
                                         examples=["292ada19-6b06-44f7-a20e-dc6687668b0e"])],
                user_service: Annotated[UserService, Depends(get_user_service)],
                book_service: Annotated[BookService, Depends(get_book_service)],
                loan_service: Annotated[LoanService, Depends(get_loan_service)]) -> LoanRead:
    try:
        book = book_service.get_if_book_available(book_id)
...
def process_loan_return_with_fine(loan_id: Annotated[UUID, UuidVersion(4), Path(description="Loan unique identifier (UUID4)")],
